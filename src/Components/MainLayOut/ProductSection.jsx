      ...
      <Slide>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((product) => (
            <div
              key={product._id}
              className="bg-[#f8f8f8] rounded-lg shadow-xl p-6 transform hover:scale-105 transition duration-300 ease-in-out"
            >
              <img
                src={product.image}
              ...
              />
              <div className="p-4">
                <h3 className="text-xl font-bold text-black">Product Name: {product.itemName}</h3>
                <p className="text-[#821131] bg-slate-100 rounded-lg px-3 py-1 text-center">Product Category: {product.categoryName}</p>
                <p className="text-sm text-black">{truncateDescription(product.description)}</p>
                ...
                <div className="mt-4">
                  <Link
                    ...
                    className="bg-transparent text-black border-2 border-black px-6 py-2 rounded-full hover:bg-black hover:text-white transition duration-300 ease-in-out"
                  >
                    View Details
                  </Link>
                </div>
